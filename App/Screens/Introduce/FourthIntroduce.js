...
import { CONG_DUC_INTRODUCE } from '../../Constants/String';

export default class FourthIntroduce extends Component {
    render() {
        return(
            <View style={styles.swipeContainer}>
                <View style={styles.headerWrapper}>
                    <Text style={styles.headerText}>Công Đức</Text>
                    {getImage('second_introduce', INTRODUCE_HEADER_WIDTH - 60, true)}
                </View>
                <View style={styles.introduceTextWrapper}>
                    <ScrollView>
                        <Text style={styles.introduceText}>{CONG_DUC_INTRODUCE}</Text>
                    </ScrollView>
                </View>
            </View>
        );
